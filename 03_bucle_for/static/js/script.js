...
agrega (concatena con +=) el número actual seguido de un guion 
(ej: "1 - 2 - 3 - ") al párrafo. */
...
Tienes la siguiente lista de estudiantes: 
let curso = ["Ana", "Diego", "Sofía", "Matias"];
Selecciona el párrafo pantalla y límpialo primero 
asignándole un texto vacío ("").
...
Tienes una lista con las notas finales de un curso: 
let notas = [3.5, 6.2, 5.0, 2.8, 4.5, 7.0];
Crea una variable contadorAprobados que inicie en 0.
Recorre el arreglo con un for. Si la nota es 4.0 o superior, 
...
Crea una variable articuloBuscado = "Cuaderno" y 
una variable vecesEncontrado = 0.
Recorre la bodega con un for. Cada vez que el elemento de la lista sea igual al 
articuloBuscado, aumenta tu contador.
...
function parrafosAdvertencia() {
    ...
    for (let i = 0; i < temperaturas.length; i++) {
        if (temperaturas[i] >= 30 && temperaturas < 50) {
            temperaturasPeligrasas.push(temperaturas[i])
            result8.innerHTML += `<p class="text-danger">¡ALERTA! Temperatura critica de 
            ${temperaturas[i]} + ${temperaturasPeligrasas} grados. </p><br>`;
        }
    }
    container.classList.remove('d-none');
}